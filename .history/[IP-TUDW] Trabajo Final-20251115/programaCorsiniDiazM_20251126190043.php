...
function resumenJugador(array $juegos, string $nombreJugador) {
    // Inicializamos contadores para victorias, empates, y acumulados de aciertos
    $ganados = 0;
    $perdidos = 0;
    $empates = 0;
    $acumulado = 0;

    // Recorremos todas las partidas de la colección de juegos
    for ($i = 0; $i < count($juegos); $i++) {
        $p = $juegos[$i];  // Accedemos a cada juego

        // Comprobamos si el jugador fue el jugador 1 o el jugador 2
        if ($p['jugador1'] == $nombreJugador) { // Si el jugador participó como jugador 1
            $acumulado = $acumulado + $p['aciertos1'];  // Sumamos sus aciertos
            if ($p['aciertos1'] > $p['aciertos2']) {
                $ganados++;  // Si ganó
            } elseif ($p['aciertos1'] < $p['aciertos2']) {
                $perdidos++; // Si perdió
            } else {
                $empates++;  // Si hubo empate
            }
        } elseif ($p['jugador2'] == $nombreJugador) {  // Si el jugador participó como jugador 2
            $acumulado = $acumulado + $p['aciertos2'];  // Sumamos sus aciertos
            if ($p['aciertos2'] > $p['aciertos1']) {
                $ganados++;  // Si ganó
    ...
    // Devolvemos el resumen con los resultados
    return [
